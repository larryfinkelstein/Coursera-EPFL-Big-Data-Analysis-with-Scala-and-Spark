...
object WikipediaRanking {
  ...
  val wikiRdd: RDD[WikipediaArticle] = sc.textFile("E:/coursera/spark/week1/wikipedia/src/main/resources/wikipedia/wikipedia.dat").map(WikipediaData.parse(_))
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    if(!map.contains(lang)){
      map(lang) = rdd.filter(a => a.text.split(" ").contains(lang)).count().toInt
    }
    map(lang)
  }
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    for(lang <- langs){
      val tmp = (lang, occurrencesOfLang(lang, rdd)) 
      list += tmp
    }
